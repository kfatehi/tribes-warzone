...
function GameBase::modifyProjData(%what, %data, %val)
{
	if(%what.bulletShapeName != "")
	{
		%string = "."@%data@"="@%val@";";
		eval(%what@%string);
		echo(%what@"."@%data@" was successfully changed to: "@%val@"!");
	}
	else
	{
		echo(%what@" is not a VALID PROJECTION Data type!");
	}
}
...
{
	if($debug)
		echo("SeekingMissile::updateTargetPercentage(%target); Has just been called!");
