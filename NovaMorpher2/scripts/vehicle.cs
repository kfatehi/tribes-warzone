...
{
	if(GameBase::getDamageLevel(%this) < (GameBase::getDataName(%this)).maxDamage && DoSpecial::Collision(%this, %object))
	{
		if (getObjectType (%object) == "Player" && %object.vehicle == "" && (getSimTime() > %object.newMountTime || %object.lastMount != %this) && %this.fading == "")
		{
			...
			if ((%armor == "larmor" || %armor == "lfemale") && Vehicle::canMount (%this, %object) || ($ArmorFly[%armor] && Vehicle::canMount (%this, %object)))
			{
			...
			else if(GameBase::getDataName(%this).shapeFile != "flyer") 
			{
...
{
	if(!DoSpecial::passengerJump(%this,%passenger,%mom))
		return;
...
//	}
	...
	for(%i = 0; %i < 7; %i++)
	{
		%mItem = Player::getMountedItem(%player, %i);
		if(%mItem != "" && (%mItem.mass*0) == 0)
			%mass += %mItem.mass;
	}
	%ratio = %mass/13; //== 13 - MArmor mass (The mass that I never modified :)
	%height = 2;
	%velocity = 100*%ratio;
	%zVel = 150*%ratio;
...
function Vehicle::dismount(%this,%mom)
{
   ...
   if(%cl != -1)
   {
	...
	%doSpecial = DoSpecial::Dismount(%thisName, %this, %mom, %cl);
	if(!%doSpecial)
	{
		return;
	}

      if(%thisName == "MorpherJet")
      {
		MorpherJet::Dismount(%this, %cl);
		return;
      }
      else if(%thisName == "MorpherPlasmaTurret")
      {
		MorpherPlasmaTurret::Dismount(%this, %cl);
		return;
      }
...
function Vehicle::onDestroyed (%this,%mom)
{
	...
	DoSpecial::OnDestroy(GameBase::getDataName(%this), %this, %cl);
...
{
	%this.lastDamager = GameBase::getControlClient(%object);
