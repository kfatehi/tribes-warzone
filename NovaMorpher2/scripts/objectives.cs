...
//========================================================================================================================//
		...
		for(%k= 0 ; %k < %numClients; %k++)
		{
			...
			// Score
			if((%clientList[%k]).score > %highScore)
			{
				%highScore = (%clientList[%k]).score;
			}

			// ScoreKills
			if((%clientList[%k]).ScoreKills > %highScoreKills)
			{
				%highScoreKills = (%clientList[%k]).scoreKills;
			}

			// ScoreDeaths
			if((%clientList[%k]).ScoreDeaths > %highScoreDeaths)
			{
				%highScoreDeaths = (%clientList[%k]).scoreDeaths;
			}

			// Ratio
			if((%clientList[%k]).ratio > %highRatio)
			{
				%highRatio = (%clientList[%k]).ratio;
			}

			// TKCount
			if((%clientList[%k]).TKCount > %highTKCount)
			{
				%highTKCount = (%clientList[%k]).TKCount;
			}
	
			// FlagCaps
			if((%clientList[%k]).FlagCaps > %highFlagCaps)
			{
				%highFlagCaps = (%clientList[%k]).FlagCaps;
			}
	
		}

//========================================================================================================================//
//================================== SORT OUT THE SCORES END!!!!! ========================================================//
//========================================================================================================================//
		...
		for(%c = 0; %index < %numClients; %c++)
		{
			...
			// scores
			%name = Client::getName(%clientList[%c]);
			%name = objective::ParseName(%name);

			%lineNum++;
			Team::setObjective(%i, %lineNum, "<L4><L3><L2<L1>");
			%lineNum++;
			Team::setObjective(%i, %lineNum, "<L4>#" @ %c @ "<L12>" @ %name @ "<L57>" @ %FontScore @ (%clientList[%c]).score @ "<L66>" @ %FontScoreKills @ (%clientList[%c]).ScoreKills @ "<L73>" @  %FontScoreDeaths @ (%clientList[%c]).ScoreDeaths @ "<L84>" @  %FontRatio @ (%clientList[%c]).ratio @ "<L92>" @  %FontTKCount @ (%clientList[%c]).TKCount @ "<L99>" @ %FontFlagCaps @ (%clientList[%c]).FlagCaps);
...
//================================================================================================ Parse Player Name
function objective::ParseName(%name)
{
	%name = escapeString(%name);
	%name = String::replace(%name, "\\", "\\\\", -0, true); //echo (" Name = " @ %name @ "");
	return %name;
}
...
//== myown :-)
function ScoreCheck(%clientId)
{
	%curLvl = floor(%clientId.TotalScore / 100);
	...
	%clientId.Rank = %curLvl;
}

//== Base MOD
function BaseGame::refreshClientScore(%clientId)
{
   %team = Client::getTeam(%clientId);
   if(%team == -1) // observers go last.
      %team = 9;
...
{
   bottomprintall("<jc><f1>" @ Client::getName(%clientId) @ " <f3>has left the game.",5);
   schedule("bottomprintall(\"<jc><f3>Is the loss of <f1>" @ Client::getName(%clientId) @ "<f3> suppost to be good?\",5);",5);
...
function zapPlayerBack(%player)
{
	%team = GameBase::getTeam(%player);
	%returnPos = GameBase::getPosition(Game::pickTeamSpawn(%team));

	returnFlag(%player); //== Remove the flag :D

	if(%player.driver == "1") //== LOL
	{
		%vehicle = Player::getMountObject(%player);
		%thisPos = GameBase::getPosition(%vehicle);
		%newPos = getWord(%returnPos, 0) @ " " @ getWord(%returnPos, 1) @ " " @ getWord(%thisPos, 2);
		GameBase::setPosition(%vehicle, %newPos);
	}
	else
		GameBase::setPosition(%player, %returnPos);

	Player::setDamageFlash(%player, 0.156);
	playSound(SoundLaserHit,%returnPos);
...
{
	%rand = getRandom() * 50;
	if(%rand < 2.1) //== 2.1 every 50 times done...
		returnFlag(%player); //== Remove the flag :D
	...
	else
	{
		...
		%newVel = Vector::Multiply(%vel, "-1 -1 1");
...
function alertPlayer(%player, %count)
{
	if(%player.outArea == 1)
	{
		if($NovaMorpher::noLeaveArea == 2)
		{
			if(%count == 6 && $NovaMorpher::noLeaveArea == 2)
			...
			else
			{
				...
				if(%player.driver == "1") //== LOL
				{
					%damPlayer = Player::getMountObject(%player);
					%type = GameBase::getDataName(%damPlayer);
					%maxHealth = %type.maxDamage;
		           		%dlevel = GameBase::getDamageLevel(%damPlayer)+(%maxHealth/10);
		           		GameBase::setDamageLevel(%damPlayer,%dlevel);
				}
				else
				{
					%armor = Player::getArmor(%player);
					%maxHealth = %armor.maxDamage;
					%damPlayer = %player;
					Player::onDamage(%damPlayer,$FireDamageType,%maxHealth/10,%pos,"","" ,"torso" ,"front_right" ,Player::getClient(%player));
				}
			}
			schedule("alertPlayer(" @ %player @ ", 0);",1.5,%player);
		}
		else
		{
			if(%count > 0)
			...
			else
			{
				...
				if($NovaMorpher::noLeaveArea)
				{
					if($NovaMorpher::noLeaveArea == 1)
						zapPlayerBack(%player);
					else if($NovaMorpher::noLeaveArea == 3)
					{
						%pos = GameBase::getPosition(%player);
						if(%player.driver == "1") //== LOL
						{
							...
							%type = GameBase::getDataName(%damPlayer);
							%maxHealth = %type.maxDamage;
				           		%dlevel = GameBase::getDamageLevel(%damPlayer)+(%maxHealth/5);
				           		GameBase::setDamageLevel(%damPlayer,%dlevel);
						}
						else
						{
...
function TowerSwitch::onCollision(%this, %object)
{
   ...
   %armor = Player::getArmor(%playerClient);
   if((%armor == "srarmor"))
   {
        Client::sendMessage(%playerClient, 1, "Cannot capture objectives with this armor! ~waccess_denied.wav");
        return;
   }
	else if(%armor == "undeadmagearmor" || %armor == "undeadmagefemale" || %playerClient.drone)
	{
	        Client::sendMessage(%playerClient, 1, "The UNDEAD cannot capture an objective!");
	        return;
	}
...
function Flag::onCollision(%this, %object)
{
   ...
   if(%this.carrier != -1)
	...
	if(Player::isAIControlled(%object) && !$NovaMorpher::EnableSpoonBot)
		return;   
   ...
   if((%armor == "srarmor") || ((Player::getItemCount(%object, "Flag") == "1" && %flagTeam != %playerTeam)))
   {
        Client::sendMessage(%playerClient, 1, "Flag slot occupied, please drop your current flag");
        return;
   }
	else if(%armor == "undeadmagearmor" || %armor == "undeadmagefemale" || %playerClient.drone)
	{
	        Client::sendMessage(%playerClient, 1, "The UNDEAD cannot carry a flag...");
	        return;
	}
      ...
      if(%object.carryFlag == "")
      {
        			...
        			if((%this.lastTeam == "" || %this.lastTeam != %playerTeam) && %flagTeam == -1)
				{
					...
					%this.lastTeam = %playerTeam;
     				      %this.timerOn = 1;
     				      if($NovaMorpher::flagToStandTime >= 30)
					{
		     			      %timeToStand = $NovaMorpher::flagToStandTime - 30;
								%timeLeft = 30;
	     				      if($NovaMorpher::flagToStandTime > 30)
	     				      	Client::sendMessage(%playerClient, 0, "You have " @ $NovaMorpher::flagToStandTime @ " sec to put the flag in a stand.");
					}
					else
					{	
						if($NovaMorpher::flagToStandTime >= 10)
							%remain = $NovaMorpher::flagToStandTime % 10;
						else 
							%remain = $NovaMorpher::flagToStandTime % 5;
							
						if(%remain > 0 && %remain != $NovaMorpher::flagToStandTime)
						{
							%timeToStand = %remain;
							%timeLeft = $NovaMorpher::flagToStandTime - %remain;
			     				Client::sendMessage(%playerClient, 0, "You have " @ $NovaMorpher::flagToStandTime @ " sec to put the flag in a stand.");
							}
						else
						{
							%timeToStand = 0;
							%timeLeft = $NovaMorpher::flagToStandTime;
						}
					}
