...
{ 	
	...
	echo(%mineTeam @ ": MineTeam\n" @ %objectTeam @ ": Object Team");

	if($NovaMorpher::FriendlyMines == "true")
	{

		if((%type == "Player" || %data == AntipersonelMine || %data == Vehicle || %type == "Moveable") && GameBase::isActive(%this) && %mineTeam != %objectTeam) 
		{
			if($debug)
				echo("Friendly Mines On, Collision, Enemie, BOOM!");
			GameBase::setDamageLevel(%this, %data.maxDamage);
		}
	}
	else if ((%type == "Player" || %data == AntipersonelMine || %data == Vehicle || %type == "Moveable") && GameBase::isActive(%this))
	{
		if($debug)
			echo("Friendly Mines Off, Collision, BOOM!");
		GameBase::setDamageLevel(%this, %data.maxDamage);
	}
}
