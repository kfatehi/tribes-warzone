...
{
   if (GameBase::virtual(%target, "getHeatFactor") >= 0.5 && %target.heatSync < 1)
      return "True";
   else
      return "False";
}
...
function Turret::onDestroyed(%this)
{
	...
	GameBase::setRechargeRate(%this,0);
	Turret::onDeactivate(%this);
	Turret::objectiveDestroyed(%this); //== I dont know why this is here...
	Turret::alertDestroyed(%this);
...
{
	//== Does nothing? I donno..
}

function Turret::alertDestroyed(%this)
{
	%player = %this.lastDamageObject;
	%clientId = GameBase::getControlClient(%player);
	%team = GameBase::getTeam(%player);
	%name = Client::getName(%clientId);

	%thisType = GameBase::getDataName(%this);
	%thisMaxDamage = %thisType.maxDamage;
	%thisName = %thisType.description;
	%thisTeam = GameBase::getTeam(%this);

	%award = floor(1+(%thisMaxDamage*2));
	%oldScore = floor(%clientId.score);

	if(%thisTeam != %team && %clientId > 2048)
	{
		%clientId.score = %oldScore + %award;
		%string = "<jc><f3>You have been awarded\n<f1> --)  "@%award@"  (--\n<f3> points for destroying a\n<f1>"@%thisName@"<f3>.";

		%radar = $Radar[%thisTeam];
		if(%radar > 0)
		{
			...
			for(%cl = Client::getFirst(); %cl != -1; %cl = Client::getNext(%cl))
			{
				...
				%playerPos = GameBase::getPosition(%curPlayer);
				%distance = floor(Vector::getDistance(%thisPos, %playerPos));
				%curTeam = Client::getTeam(%cl);
				if(%thisTeam == %curTeam)
					Client::sendMessage(%cl, 1, %thisName@" ID#"@%this@" was destroyed by "@%name@" "@%distance@" meters away from you!");
			}
		}
	}
	else
	{
		%clientId.score = %oldScore - %award;
		%string = "<jc><f3>You have been PUNISH\n<f1> --)  "@%award@"\  (--\n<f3> points for destroying your team's\n<f1>"@%thisName@"<f3>.";
	}

	bottomPrint(%clientId, %string, 10);
	if((%clientId.score*0) != 0 || %clientId.score == "+INF") //== LOL +INF!!!
		%clientId.score = 0;
...
function Turret::onDamage(%this,%type,%value,%pos,%vec,%mom,%object)
{
	...
	%DamageScale = $damageScale[GameBase::getDataName(%this), %type];
	if(%DamageScale == "")
		%DamageScale = 1.0;
	%totalvalue = (%DamageScale * %value * %TDS);
	StaticShape::shieldDamage(%this,%type,%totalvalue,%pos,%vec,%mom,%object);
}

function Turret::onControl (%this, %object)
{
	%client = Player::getClient(%object);
	Client::sendMessage(%client,3,"Controlling turret " @ %this);
}

function Turret::onDismount (%this, %object)
{
	%client = Player::getClient(%object);
	Client::sendMessage(%client,1,"Leaving turret " @ %this);
}

function Turret::onCollision (%this, %object)
{
	if($Turret::CanManualControl[%name])
	{
		%clientId = GameBase::getControlClient(%object);
		%player = Client::getOwnedObject(%clientId);
		%speed = velocity::getMPH(velocity::get(%player));
		if(%speed < 100) //== If you are going too fast, infer that it is an accidental hit.
		{
			if (getObjectType(%player) == "Player")
			{
				//== Kick da person LOL
				%controlerId = GameBase::getControlClient(%this);
				%controlerPlayer = Client::getOwnedObject(%couuntrolerId);
				if(%controlerId != -1)
				{
					Client::sendMessage(%controlerId, 1, "Manual OVERRIDE detected!");
					Client::setGuiMode(%controlerId,2);
					Client::setControlObject(%controlerId, %controlerPlayer);
				}
