...
function StaticShape::onDamage(%this,%type,%value,%pos,%vec,%mom,%object)
{
	...
	if(GameBase::getTeam(%this) == GameBase::getTeam(%object))
	{
		...
		{
			if($StaticShape::NoTDEffect[GameBase::getDataName(%this)])
				%dValue = %damageLevel + %value * %TDS;
			else
				%dValue = %damageLevel + %value;
		}
	...
	if($StaticShape::NoSpecialEffect[%type]){}
	else
	{
		%shooterClient = %object;
	 	%shooterPlayer = Client::getOwnedObject(%shooterClient);
	 	%damagedObj = %this;

		if(%shooterClient > 2048 && %damagedObj > 0) //== Make sure no bull !@#$ comes along....
			DoSpecialDamageCheck(%shooterPlayer, %damagedObj, %type, %shooterClient);
	}
...
function Generator::allDown(%team)
{
	%team.genDown = true;
	%team.backUpPower = "2"; //== 5 is MORE then Enough
	%team.backUpPower2 = "5"; //== 10 is MORE then Enough
}
...
function Generator::onEnabled(%this)
{
	...
	if($origTeam[%this])
		%thisTeam = $origTeam[%this];
	else
		%thisTeam = GameBase::getTeam(%this);

	TeamBottomPrintAll(%thisTeam,"<jc><f2>Generator<f3> ID#" @ %this @ " has been activated!");

	%thisTeam.genDown = false;
	%thisTeam.backUpPower = "0";
}
...
function Generator::onDisabled(%this)
{
	...
	if(!findAtvGen(%thisTeam))
		Generator::allDown(%thisTeam);
}
...
function Generator::onDestroyed(%this)
{
	...
	TeamBottomPrintAll(%thisTeam,"<jc><f1>GENERATOR<f3> ID#" @ %this @ " HAS BEEN <f0>DESTROYED<f3>!!!");
...
function TowerSwitch::onDestroyed(%this)
{
	messageAll(1,"TOWER SWITCH DESTROYED! Fortunatly it was AUTO-REPAIRED!!!! >) Nice try...");
...
function CargoCrate::onCollision(%this, %object)
{
	if(getObjectType(%object) == "Player")
	{
		%cnt = Station::itemsToResupply(%object);
		if(%cnt)
		{
			%clientId = Player::getClient(%object);
			%word = radnomItems(3, "resources", "ammo", "raw material");
			client::sendmessage(%clientId,3,"Hey you found some "@%word@"!");
			schedule("StaticShape::shieldDamage("@%this@","@1000@","@%cnt/100@",'','','',"@%object@");",-0);
		}
	}
}
...
function CargoBarrel::onCollision(%this, %object)
{
	if(getObjectType(%object) == "Player")
	{
		%cnt = Station::itemsToResupply(%object);
		...
		{
			%clientId = Player::getClient(%object);
			%word = radnomItems(4, "oil", "gas", "ammo", "explosives");
			client::sendmessage(%clientId,3,"Hey you found some "@%word@"!");
			schedule("StaticShape::shieldDamage("@%this@","@1000@","@%cnt/100@",'','','',"@%object@");",-0);
		}
	}
